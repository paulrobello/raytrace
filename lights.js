Partrace.Light=BaseObj.extend({
  init: function(partrace,parent){
    ...
    this.ka=vec4.fromValues(0.1,0.1,0.1,1);
...
Partrace.Lights.Point=Partrace.Light.extend({
  ...
  intensity:function(color,ip){
    ...
    if (ldotn<Partrace.epsilon) return;
    ...
    if (sh>0){
      ...
      if (rdotv>0){ // specular
        ...
        vec4.multiply(sl,sl,mat.s); // add material specular
    ...
    switch (this.attenuationType){
      ...
      case 'squared':att=1-Math.clamp(Math.sqr(dist/this.fallOffRadius),0,1); break;
    ...
    vec4.scale(color,color,mat.d[3]); // apply transparency       
  }    
});
